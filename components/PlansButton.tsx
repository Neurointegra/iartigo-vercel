...
export function PlansButton({ 
  ...
  const getPlansRoute = () => {
    // Se o usuário tem um plano ativo (não "Por Artigo" e não cancelado), vai para gerenciamento
    if (user?.plan && user.plan !== 'Por Artigo' && user.subscriptionStatus !== 'cancelled') {
      return '/plans/manage' 
    } else {
      return '/plans' // Usuário sem plano, plano "Por Artigo" ou plano cancelado vai para escolha de planos
    }
  }
